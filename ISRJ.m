function [sig_isrj] = ISRJ(B,Tp,pri,fs,f0,pluse_num,c,target_p,target_v,min_K,max_K,min_TL,max_TL,min_jsr,max_jsr,sig)
%ISRJ 间歇采样重复转发干扰
%   K为重复转发次数 TL为切片宽度
    K = B/Tp;
    t = (0:1/fs:pri-1/fs).';
    sig_isrj = zeros(length(t),pluse_num);
    ...
    isrj_jsr = (max_jsr - min_jsr) * rand() + min_jsr;
    isrj_jsr_amp = 10.^(isrj_jsr/10);
    ...
    for i = 1:pluse_num
        ...
        if(end_num>length(t)) %超过末尾的点数
            ...
            sig_isrj(start_num:end,i) = isrj_jsr_amp * sig_buffer(1:valid_point);
        else
            sig_buffer = sig(start_num:end_num,i);
            sig_buffer = reshape(sig_buffer,(K+1)*TL_num,H);
            sig_buffer = repmat(sig_buffer(1:TL_num,:),K+1,1);
            sig_buffer = reshape(sig_buffer,H*(K+1)*TL_num,1);
            sig_isrj(start_num:end_num,i) = isrj_jsr_amp * sig_buffer;
        end
    end
